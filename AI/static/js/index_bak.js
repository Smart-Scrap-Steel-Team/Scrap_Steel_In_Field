...
//setInterval(updateSegmentationStats, 1000)
      // 每秒更新一次所有图片和数据
      // 注意：监控图片已替换为实时视频流，不再使用checkForNewMonitorImage
      setInterval(updateSegmentationStats,1000)//setInterval(updateSegmentationStats,1000)秒更新一次分割统计数据,调用一次 updateSegmentationStats 函数，刷新图像分割的统计结果使结果快速准确
      setInterval(directLoadJsonFile,1000)//setInterval(directLoadJsonFile,1000)用 directLoadJsonFile 函数，直接加载最新的 JSON 数据文件
      setInterval(updateArmStatus,2000)//setInterval(updateArmStatus,2000)每2秒调用一次 updateArmStatus 函数，更新机械臂的状态信息('我们用最轻量、最兼容的 Web 技术，搭建了一套工业级实时监控系统，降低了开发成本和部署门槛')
