    ...
    fetch('/arm_status')
        ...
        .then(data => {
            ...
            if (data.arm1_status === true) {
                ...
                // 复位启动按钮
                const btn = document.getElementById('startArmBtn');
                if (btn) {
                    btn.textContent = '启动机械臂';
                    btn.style.pointerEvents = 'auto';
                    btn.style.opacity = '1';
                    btn.style.backgroundColor = 'rgba(0, 244, 253, 0.1)';
...
// 启动机械臂：调用后端接口，后端开启TCP服务器等待机械臂连接
// 收到 yes_grab 后发送三个坐标
function startArm() {
    const btn = document.getElementById('startArmBtn');
    btn.textContent = '启动中...';
    ...
    fetch('/start_arm', { method: 'POST' })
        .then(r => r.json())
        .then(data => {
            if (data.success) {
                console.log('机械臂启动成功:', data.message);
                if (data.points) {
                    console.log('将发送的坐标:', data.points);
                }
                btn.textContent = '运行中';
                btn.style.backgroundColor = 'rgba(0, 244, 253, 0.2)';
                btn.style.boxShadow = '0 0 12px rgba(0, 244, 253, 0.5)';
                btn.style.pointerEvents = 'none';
                btn.style.opacity = '1';
            } else {
                console.error('机械臂启动失败:', data.message);
                btn.textContent = '启动机械臂';
                btn.style.pointerEvents = 'auto';
                btn.style.opacity = '1';
...
// 停止机械臂
function stopArm() {
    fetch('/stop_arm', { method: 'POST' })
        .then(r => r.json())
        .then(data => {
            if (data.success) {
                console.log('机械臂已停止:', data.message);
            }
        })
        .catch(error => {
...
document.addEventListener('DOMContentLoaded', function() {
    const startBtn = document.getElementById('startArmBtn');
    if (startBtn) {
        startBtn.addEventListener('click', startArm);
    }
});
...
function startLiveVehicleStream() {
    const monitorImageDiv = document.getElementById('monitorImage');
    const videoStreamImg = document.getElementById('vehicleVideoStream');

    if (!monitorImageDiv) {
        console.error("找不到monitorImage元素");
        return;
    }

    console.log("正在启动车辆实时视频流...");

    // 确保元素可见
    monitorImageDiv.style.display = 'flex';
    monitorImageDiv.style.visibility = 'visible';
    monitorImageDiv.style.opacity = '1';

    // 如果HTML中已经有videoStreamImg元素，确保它使用正确的src
    if (videoStreamImg) {
        console.log("使用HTML中定义的RTSP视频流");
        videoStreamImg.style.display = 'block';
        isLiveStreamActive = true;
    } else {
        // 备用方案：动态创建视频流元素
        monitorImageDiv.innerHTML = `
            <h3>车辆图像</h3>
            <img id="vehicleVideoStream" src="/video_feed" alt="车辆实时视频流"
                 style="width:100%; height:100%; object-fit:contain; display:block;"
                 onerror="handleLiveStreamError()">
        `;
        isLiveStreamActive = true;
        console.log("车辆实时视频流已启动，src=/video_feed");
    }
}
...
function checkForNewSegmentationImage() {
  return fetch('update_segmentation', {
      method: 'POST'
  })
  .then(response => response.json())
  .then(data => {
  ...
  .catch(error => {
      ...
      if(!imgElement) {
          document.getElementById('segmentationImage').innerHTML =
              `<h3>分割结果</h3><img id="segmentationImg" src="/get_segmentation?t=${new Date().getTime()}"
...
// 数字动画递增效果
function animateValue(elementId, newValue, isFloat = false, duration = 1500) {
    ...
    fetch('/get_segmentation_stats')
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // 更新方管类 - 使用动画效果
                if (stats['方管类']) {
                    animateValue('squareTubeCount', stats['方管类'].count);
                } else {
                    document.getElementById('squareTubeCount').textContent = '0';
                }

                // 更新圆管类
                if (stats['圆管类']) {
                    animateValue('roundTubeCount', stats['圆管类'].count);
                } else {
                    document.getElementById('roundTubeCount').textContent = '0';
                }

                // 更新钢板类
                if (stats['钢板类']) {
                    animateValue('steelPlateCount', stats['钢板类'].count);
                } else {
                    document.getElementById('steelPlateCount').textContent = '0';
                }

                // 更新镀锌件
                if (stats['镀锌件']) {
                    animateValue('galvanizedCount', stats['镀锌件'].count);
                } else {
                    document.getElementById('galvanizedCount').textContent = '0';
                }

                // 更新钢筋
                if (stats['钢筋']) {
                    animateValue('rebarCount', stats['钢筋'].count);
                } else {
                    document.getElementById('rebarCount').textContent = '0';
                }

                // 更新管道连接件
                if (stats['管道件']) {
                    animateValue('pipeFittingCount', stats['管道件'].count);
                } else {
                    document.getElementById('pipeFittingCount').textContent = '0';
                }

                // 更新金属连接件
                if (stats['金属件']) {
                    animateValue('metalFittingCount', stats['金属件'].count);
                } else {
                    document.getElementById('metalFittingCount').textContent = '0';
                }

                // 更新打孔废料
                if (stats['打孔废料']) {
                    animateValue('scrapCount', stats['打孔废料'].count);
                } else {
                    document.getElementById('scrapCount').textContent = '0';
                }

                // 总数和总面积使用动画效果
                animateValue('totalCount', data.total_count);
                animateValue('totalAreaCount', data.total_area, true);
            } else {
...
window.onload = function() {
  ...
  Promise.all(initialLoads)
    .then(() => {
      ...
      // 每10秒更新一次所有图片和数据
      // 注意：监控图片已替换为实时视频流，不再使用checkForNewMonitorImage
      setInterval(checkForNewDangerImage, 10000);
      setInterval(checkForNewSegmentationImage, 10000);
      setInterval(updateSegmentationStats, 10000);  // 每秒更新一次分割统计数据
      setInterval(directLoadJsonFile, 10000);
      setInterval(updateArmStatus, 20000);

      // 每20秒检查一次文件是否更新 (这些会触发饼图和数量更新)
      setInterval(checkSegmentationFileUpdate, 20000);
      setInterval(checkDangerFileUpdate, 20000);
    })
    .catch(error => {
