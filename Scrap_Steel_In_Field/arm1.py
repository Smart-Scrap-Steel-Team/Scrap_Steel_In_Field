...
from Scrap_Steel_In_Field.config.arm_config import LOG_CONFIG, ARM_CONFIG
from Scrap_Steel_In_Field.config.detection_config import SAVE_CONFIG
...
from Scrap_Steel_In_Field.inference.detection_inference_simplified import simple_detect
from Scrap_Steel_In_Field.inference.scrap_segmentation import simple_segment
from Scrap_Steel_In_Field.get_image.field_light import on_field_light
# from Scrap_Steel_Loading.corner_coord_detect.arm_coords import detect_and_transform
from Scrap_Steel_In_Field.corner_coord_detect.detect_and_transform import calculate_grasp_points,detect_and_transform_rectangle
import json
import matplotlib
        ...
        while True:
            ...
            if msg=="yes_grab":
                ...
                if success:
                    ...
                    # # 更新 Flask 应用中的图片路径
                    # try:
                    #     response = requests.post('http://localhost:5000/set_image_path', 
                    #                         json={'path': image_path})
                    #     if response.json()['success']:
                    #         logger.info("图片路径已更新到 Flask 应用")
                    #     else:
                    #         logger.warning("更新图片路径到 Flask 应用失败")
                    # except Exception as e:
                    #     logger.error(f"更新图片路径到 Flask 应用时出错: {str(e)}")
