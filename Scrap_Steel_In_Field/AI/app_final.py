...
# 导入推理模块
from inference.detection_inference_simplified import DetectionManager, simple_detect
from inference.scrap_segmentation import SegmentationManager, simple_segment
from config.detection_config import MODEL_CONFIG as DETECT_CONFIG
from config.segment_config import MODEL_CONFIG as SEGMENT_CONFIG
...
def run_inference_loop():
    """后台推理循环，每隔5秒执行一次推理"""
    global last_inference_time, detector, segmenter
...
def _capture_thread(rtsp_url):
    ...
    cap.set(cv2.CAP_PROP_BUFFERSIZE)#cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
...
# 机械臂相关全局变量
ARM_SERVER_PORT = 8870  # 机械臂连接端口（与 test_arm_online.py 一致）
...
@app.route('/start_arm', methods=['POST'])
def start_arm():
    """启动机械臂：拍照 → 计算抓取坐标 → 发送坐标给机械臂"""
    global arm_connected, arm_cycle_active, arm_client_socket
