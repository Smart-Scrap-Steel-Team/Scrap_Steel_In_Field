...
def save_and_visualize_results(original_image, detections, detection_dir, categories):
    """保存并可视化检测结果
...
class DetectionManager:
    ...
    def save_results(self, detections, original_image):
        """保存检测结果
        ...
        return save_and_visualize_results(
            original_image, detections, self.detection_dir, self.categories)
...
def simple_detect(image_path, config=None):
    ...
    try:
        ...
        if detections is not None:
            ...
            if not detections:
                # 保存原始图像
                cv2.imwrite(detection_path, cv2.cvtColor(original_image, cv2.COLOR_RGB2BGR))
                detections = [{
                    "type": "detection",
                    "label": "NoDangerous",
                    "bbox": [0, 0, 0, 0],
            ...
            # else:
            #     # 保存带检测框的结果
            #     detector.save_results(detections, original_image)
            #     # 将检测结果路径添加到每个检测结果中
            #     for det in detections:
